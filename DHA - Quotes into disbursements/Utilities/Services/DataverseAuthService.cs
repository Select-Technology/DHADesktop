    ...
    public class DataverseAuthService
    {
        ...
        private async Task<bool> ConnectInternalAsync(bool forceReconnect = false)
        {
            try
            {
                ...
                // Only dispose and clear cache when explicitly forced to reconnect
                if (forceReconnect)
                {
                    System.Diagnostics.Debug.WriteLine("Force reconnect - clearing token cache and disposing client");
                ...
                {
                    // Try to use cached tokens first - no prompt
                    connectionString = $"AuthType=OAuth;" +
                                     $"Url={EnvironmentUrl};" +
                                     $"AppId={ClientId};" +
                                     $"RedirectUri={RedirectUri};" +
