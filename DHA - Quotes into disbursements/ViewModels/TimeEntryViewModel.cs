    ...
    public class TimeEntryViewModel
    {
        ...
        public async Task<Guid?> SaveTimeEntryAsync(TimeEntry timeEntry)
        {
            try
            {
                ...
                {
                    // Use the original CreateTimeEntry method that returns Guid
                    var newId = _timeEntryService.CreateTimeEntry(timeEntry);
                    return newId != Guid.Empty ? newId : (Guid?)null;
                }
            }
            catch (Exception)
            {
        ...
        /// <summary>
        /// Saves a time entry and returns the fully populated entry with project number and client name
        /// </summary>
        /// <param name="timeEntry">Time entry to save</param>
        /// <returns>The saved time entry with all data populated, or null if operation failed</returns>
        public async Task<TimeEntry> SaveTimeEntryWithDataAsync(TimeEntry timeEntry)
        {
            try
            {
                if (_dataverseService != null)
                {
                    // Save using DataverseService
                    var newId = await _dataverseService.SaveTimeEntryAsync(timeEntry);
                    if (newId.HasValue && newId.Value != Guid.Empty)
                    {
                        // Get the full entry with joined data
                        var allEntries = GetTimeEntries();
                        return allEntries.FirstOrDefault(te => te.Id == newId.Value);
                    }
                }
                else
                {
                    // Use the new CreateTimeEntryWithData method that returns full TimeEntry
                    var newId = _timeEntryService.CreateTimeEntry(timeEntry);
                    if (newId != Guid.Empty)
                    {
                        timeEntry.Id = newId;
                        timeEntry.IdGuid = newId;
                        return timeEntry;
                    }
                    return null;
                }
        ...
        public bool ValidateTimeEntry(TimeEntry timeEntry, out string errorMessage)
        {
            ...
            // Check that either project or quote is selected based on classification
            if (timeEntry.Classification == TimeEntryClassification.Project)
            {
            ...
            else if (timeEntry.Classification == TimeEntryClassification.Quote)
            {
                if (timeEntry.QuoteId == Guid.Empty)
                {
                    errorMessage = "Please select a quote.";
                    return false;
                }
            }
            else
            {
                // If classification is somehow not set properly, require either project or quote
                if (timeEntry.ProjectId == Guid.Empty && timeEntry.QuoteId == Guid.Empty)
                {
                    errorMessage = "Please select either a project or a quote.";
                    return false;
                }
            }
